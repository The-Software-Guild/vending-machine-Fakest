...
public class UserIOConsoleImpl implements UserIO {
    ...
    public BigDecimal readBigDecimal(String msgPrompt) {
        ...
        BigDecimal input = new BigDecimal(0.00).setScale(2, RoundingMode.HALF_UP);
        while(invalidInput){
            try{
                this.print(msgPrompt);
                input = console.nextBigDecimal().setScale(2, RoundingMode.HALF_UP);
                console.nextLine();
                invalidInput=false;

            } catch (NumberFormatException e) {
                this.print("Input error - Please enter value in format #.##");
            }
        }
